...
import { IDayStoryItem } from '@/types/story';
...
const StoryBottom = ({
  ...
  return (
    <div className="text-white absolute bottom-0 w-full h-fit flex flex-row justify-between p-5 text-xs bg-gradient-to-t from-black/40 z-10">
      <div className="flex flex-row gap-4">
        <div className="grid justify-items-center" onClick={showReactHistory}>
          ...
          <div>반응 보기</div>
          {dayStoryList.reactions.length > 0 && (
            <div className="absolute w-24 h-auto bottom-14 left-0 py-2">
              <FloatingHeart />
            </div>
          )}
        </div>
